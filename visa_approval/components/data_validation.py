...
from visa_approval.entity.artifact_entity import DataValidationArtifact, DataIngestionArtifact
...
class DataValidation:
    ...
    def validate_number_of_columns(self, data_frame: DataFrame) -> bool:
        
        try:
            
            status = len(data_frame.columns) == len(self._schema_config["columns"])
            return status
        except Exception as e:
            raise USvisaException(e, sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                validation_error_msg += "Training data frame does not have all required columns.\n"
                
            status = self.validate_number_of_columns(test_df)
            logging.info(f"All required columns are present in testing dataframe: {status}")
            if not status:
                validation_error_msg += "Testing data frame does not have all required columns.\n"
            ...
            if validation_status:
                ...
                if drift_status:
                    ...
                    validation_error_msg += "Data drift detected between training and testing dataframes.\n"
                else:
                    logging.info("No data drift detected")
                    validation_error_msg += "No data drift detected between training and testing dataframes.\n"
            else:
                logging.info("Data validation failed due to missing columns or incorrect number of columns")
